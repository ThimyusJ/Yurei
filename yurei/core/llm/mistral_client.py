...
class MistralClient:
    ...
    def __init__(self, url: str = OLLAMA_URL, model: str = OLLAMA_MODEL, timeout: int = 90):
        self.url = url.rstrip("/")
        self.model = model
    ...
    def infer(self, text: str) -> Dict[str, Any]:
        ...
        canidate = _strip_json(content) or content
        try:
            data = json.loads(canidate)
        except Exception:
        ...
        s = data["slots"]
        for k, v in {
            "verbose": False, "aggressive": False, "udp": False, "http": False, "smb": False, "vuln": False, "save": False, "consent": False, "mode": "udp" if s.get("udp") else "tcp",
        }.items():
            s.setdefault(k, v)
